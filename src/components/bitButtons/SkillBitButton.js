...
const SkillBitButton = ({ navigation }) => {
  const { state, fetchSkillStatus } = useContext(SkillContext)
  ...
  const renderStatus = () => {
    const { loading, skillStatus } = state
    if (loading)
  ...
  return (
    <>
      <NavigationEvents
        onWillBlur={fetchSkillStatus}
        onWillFocus={fetchSkillStatus}
      />
...
export default withNavigation(SkillBitButton)
