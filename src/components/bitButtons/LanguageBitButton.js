...
const LanguageBitButton = ({ navigation }) => {
  const { state, fetchLanguageStatus } = useContext(LanguageContext)
  ...
  const renderStatus = () => {
    const { loading, languageStatus } = state
    if (loading)
  ...
  return (
    <>
      <NavigationEvents
        onWillBlur={fetchLanguageStatus}
        onWillFocus={fetchLanguageStatus}
      />
...
export default withNavigation(LanguageBitButton)
