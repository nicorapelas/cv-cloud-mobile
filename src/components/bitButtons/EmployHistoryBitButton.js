...
const EmployHistoryBitButton = ({ navigation }) => {
  const { state, fetchEmployHistoryStatus } = useContext(EmployHistoryContext)
  ...
  const renderStatus = () => {
    const { loading, employHistoryStatus } = state
    if (loading)
  ...
  return (
    <>
      <NavigationEvents
        onWillBlur={fetchEmployHistoryStatus}
        onWillFocus={fetchEmployHistoryStatus}
      />
...
export default withNavigation(EmployHistoryBitButton)
