...
const AttributeBitButton = ({ navigation }) => {
  const { state, fetchAttributeStatus } = useContext(AttributeContext)
  ...
  const renderStatus = () => {
    const { loading, attributeStatus } = state
    if (loading)
  ...
  return (
    <>
      <NavigationEvents
        onWillBlur={fetchAttributeStatus}
        onWillFocus={fetchAttributeStatus}
      />
...
export default withNavigation(AttributeBitButton)
