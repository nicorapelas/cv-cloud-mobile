...
const SecondEduBitButton = ({ navigation }) => {
  const { state, fetchSecondEduStatus } = useContext(SecondEduContext)
  ...
  const renderStatus = () => {
    const { loading, secondEduStatus } = state
    if (loading)
  ...
  return (
    <>
      <NavigationEvents
        onWillBlur={fetchSecondEduStatus}
        onWillFocus={fetchSecondEduStatus}
      />
      <View style={styles.bed}>
        <TouchableOpacity
          style={styles.button}
          onPress={() => navigation.navigate('SecondEdu')}
        >
          <Text style={styles.title}>secondary education</Text>
        </TouchableOpacity>
        {renderStatus()}
      </View>
    </>
...
export default withNavigation(SecondEduBitButton)
